...
export default function ProductsPage() {
  ...
  const themeColor = mode === 'live' ? '#e2136e' : '#3b82f6';
  ...
  return (
    <div className="min-h-screen bg-[#f3f4f6] py-20 px-4 transition-colors duration-500">
      <div className="max-w-6xl mx-auto">
        <header className="text-center mb-16">
          <div className={`inline-block px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest mb-4 bg-white shadow-sm border ${
            mode === 'live' ? 'text-[#e2136e] border-[#e2136e20]' : 'text-blue-500 border-blue-200'
          }`}>
            {mode} Mode Active
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {PRODUCTS.map((product) => (
            <div 
              key={product.id}
              className="bg-white rounded-[2.5rem] p-8 shadow-sm hover:shadow-2xl transition-all duration-500 border border-gray-100 flex flex-col items-center text-center group"
            >
              ...
              <div className={`text-3xl font-black mb-8 transition-colors ${textClass}`}>
              ...
              <button
                ...
                className={`w-full py-4 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all ${
                  loading === product.id 
                    ...
                    : `${themeClass} text-white active:scale-95 shadow-lg shadow-black/5`
                }`}
              >
                {loading === product.id ? (
        ...
        <div className="mt-20 text-center">
            <Link href="/" className="text-gray-500 hover:text-gray-900 font-semibold flex items-center justify-center gap-2 group transition-colors">
